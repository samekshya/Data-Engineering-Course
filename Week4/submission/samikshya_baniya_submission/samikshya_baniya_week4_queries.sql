...
-- the fare (base fare x surge + tip - discount) and extract year/month
-- from the raw timestamp. In the warehouse these were already computed
-- during the ETL, so the query was simpler even with one extra join.
...
-- of all trips in the same query — no separate query is needed for
-- the overall total.
--   COUNT(*)                  -> trips in each hour (because of GROUP BY)
--   SUM(COUNT(*)) OVER ()     -> total trips across ALL hours
--   an empty OVER () means: apply this across the whole result set,
